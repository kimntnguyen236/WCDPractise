...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      tbUser user = dao.checklogin(uname);
        if (user.getUserName()!=null) {
            if (user.getPassword().equals(upas)) {
                   HttpSession session = request.getSession();
                    session.setAttribute("us", dao.checklogin(uname).getUserName());
                response.sendRedirect("EplServlet");
            } else {
                out.println("<h5 style=\"color: red\">Invalid password</h5>");
                request.getRequestDispatcher("Login.jsp").include(request, response);
            }
        } else {
            out.println("<h5 style=\"color: red\">Invalid username</h5>");
            request.getRequestDispatcher("Login.jsp").include(request, response);
        }
        }
    }
